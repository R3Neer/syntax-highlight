...
const DEFAULT_PREPARED_HIGHLIGHT = prepareHighlightConfig(
  DEFAULT_HIGHLIGHT_CONFIG,
);
const OPERATOR_CATEGORY =
  DEFAULT_HIGHLIGHT_CONFIG.categories["operator-symbol"];
const MUD_OPERATORS = new Set(
  DEFAULT_PREPARED_HIGHLIGHT.symbols
    .filter(({ categoryId }) => categoryId === OPERATOR_CATEGORY)
    .map(({ text }) => text),
);
