...
export function settingsDocument(settings: SyntaxPluginSettings): SettingsDocument {
  ...
  return { kind: "syntax-highlight-settings", schemaVersion: 2, settings: copy };
}
...
export function languageDocument(
  ...
  return {
    ...
    language: {
      profile: exported,
      grammars: { lexical: lexicalGrammar, syntax: syntaxGrammar },
    },
...
export function parsePortableDocument(source: string): PortableDocument {
  ...
  if (object.schemaVersion === 2) return value as PortableDocument;
  if (object.kind === "syntax-highlight-settings") {
    return { ...object, schemaVersion: 2 } as unknown as SettingsDocument;
  }
  if (object.kind === "syntax-highlight-theme") {
    return { ...object, schemaVersion: 2 } as unknown as ThemeDocument;
  }
  const legacy = object as {
    profile: LanguageProfileSettings;
    lexicalGrammar?: string;
    syntaxGrammar?: string;
    themes?: ThemePreset[];
  ...
  return {
    ...
    language: {
      ...
      grammars: {
        lexical: legacy.lexicalGrammar,
        syntax: legacy.syntaxGrammar,
      },
    },
